...
per-área YTD from the **production** `assemble_dre_sections`. Materiality **R$ 1.000 on the
YTD *or* on any single month** ("R$ 4,80 does not matter, R$ 1.900 does"); the smaller ones
are summed as a named remainder rather than dropped. One format per entry: *planilha value +
...
about in its own second paragraph. Deltas are also now `round(ours) − round(theirs)` via one
`_delta` helper, because the previous `round(ours − theirs)` made Σ(printed months) ≠ printed
Acumulado — *Contencioso · Custo equipe* printed **+3.140,19 in the summary and +3.140,20 in
its own detail table**. Both are guarded by `tests/test_diferencas_doc.py` (9 tests), the
first tests this generator has ever had.

**16 material differences, all attributed, all in despesa.** (Was "10 … institucional
Resultado Bruto differs +R$131,84" — both stale: the count moved with the or-any-month rule
above, and the RB gap is **−R$ 5.003,04**, per `convenio-parte-mbc-derived`. Corrected
2026-08-05.) Receita (−R$0,60 on 2,1M), Impostos (−R$0,09) and Amortização (R$0,00) tie.
Four causes account for everything: the workbook's own `r204/205/206` one-row formula shift
(Jan–Mai), the same shift reaching *Despesa Institucional* through `r203` (the rateio pool is
`r198 − r203`), Vale in Custo equipe — where the workbook's basis varies by month on both the
lawyers (r26/27, r56/57 zeroed in mar/abr/mai) and the ADM (r122/123, three different bases)
— and the jan/fev convênio memo, now **derived** rather than awaiting a ruling.
