...
| `LDESK.DB_VW_FATURASEMI_REC` (view) | **Faturas emitidas + recebimento, com moeda** — a fonte das abas **Nacional** (BRL) e **Moedas** (EUR/USD) | **DESCOBERTO (2026-07-14 probe_nacional_moedas):** `FAT_FATURA` cru NÃO serve (mai honorários 774.917,10, 53 linhas incl. cancelada `SITUACAO='C'`, ≠ sacred 719.988,05). `FAT_FATURA` tem `NUMERO`, `ID_MOEDA`, `RAZAO_SOCIAL`, `DATA_EMISSAO`, `DATA_VENCIMENTO`, `VALOR_HONORARIOS/DESPESAS/DESCONTO`, `COTACAO`, `GROSSUP_HON`, `ID_PROFISSIONAL_RESP`, `SITUACAO` — mas SEM data de recebimento nem cliente-nome nem NF. A view **`DB_VW_FATURASEMI_REC`** carrega `SIGLA_MOEDA` + `SIGLA_MOEDA_NACIONAL` (emitidas COM recebimento + conversão) = exatamente as 16 colunas das abas. Cliente-nome via `CAD_CLIENTE.ID_PESSOA`→`CAD_PESSOA.NOME` (2-hop confirmado). Moeda: `CAD_MOEDA.ID_MOEDA` (R/E/U). **PRÓXIMO:** probe as colunas de `DB_VW_FATURASEMI_REC` antes de escrever o bloco T8. |
