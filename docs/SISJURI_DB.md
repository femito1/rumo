...
| `030.010.0160` | **ISS jurídico — TRIMESTRAL** (posta só em quarter-ends: jan/abr/jul/out; ZERO em mai/jun, por isso o tie de maio nunca pegou). O workbook o lança **dentro de Custo equipe por área** ("ISS Trimestral", Base_Resultado linhas 25/54/79). Jan `5.350,24` (Conten 1.719,72 + Econ 2.101,88 + Arb 1.528,64 ✓) · Abr `5.578,58`. | 🐛 **BUG (2026-07-21, probe_unknown_accounts):** hoje é DESCARTADO — `is_imposto()` retorna True (token "iss") → não entra em Custo equipe; e a linha Imposto do DRE é 15%×recebimento (fórmula), então não entra lá tampouco. Jan/Abr custo equipe fica ~5k curto → células blankam. **Fix:** ler `030.010.0160` no `custo_equipe_deriv` por SIGLADEST (como as outras 030 de equipe) e excluir de `is_imposto`. Ver `FINDINGS_2026-07-21-manuais-refutados.md`. |
