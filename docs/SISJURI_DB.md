...
| `030.010.0160` | **ISS jurídico — TRIMESTRAL** (posta só em quarter-ends: jan/abr/jul/out; ZERO em mai/jun, por isso o tie de maio nunca pegou). O workbook o lança **dentro de Custo equipe por área** ("ISS Trimestral", Base_Resultado linhas 25/54/79). Jan `5.350,24` · Abr `5.578,58` · Jul `5.578,58`. Por advogado: jan 382,16/cada (JGS 2×). | ✅ **CORRIGIDO (2026-07-21, b850300+73ef7a3):** era DESCARTADO (`is_imposto()` True pelo token "iss" → fora de Custo equipe; linha Imposto do DRE é 15%×receb, então não entrava lá tampouco). Fix: `is_imposto` exclui qualquer `is_direct_team`; ISS entra por advogado no `custo_equipe_deriv` (GERENC por `ID_PROFISSIONAL`→sigla) e faz fold pelo home-grupo + rateio AM 50/50. **BATE AO CENTAVO, incl. split Econ↔Arb — 100% derivável (2026-07-21).** ⭐ A área de cada unidade de ISS = a **home area do `LANCSOLICITANTE`** (solicitante), NÃO do `LANCPROFDEST`, depois foldada pelo mesmo rateio AM 50/50. Provado diffando as 70 colunas das 2 linhas do JGS (`probe_iss_jgs_allcols`): idênticas EXCETO `LANCSOLICITANTE` (JGS vs **MAM**), ambas fatias `DESNITEM` do mesmo `CPGCNUMEROPAGAR=18172`. MAM=Econômico → a 2ª unidade do JGS cai em Econômico. `probe_iss_solicitante` #O + rateio AM: Conten **1.719,72** / Econ **2.101,88** / Arb **1.528,64** = workbook ao centavo. A rota antiga (GERENC por profD) colapsava tudo p/ home group — era o que fazia parecer "manual". Extract agora chaveia ISS por `LANCSOLICITANTE` (FINANCE.LANCAMENTO). Locked: `test_iss_juridico_ties_workbook_via_solicitante`. |
| `030.010.0200` | Seguro de Vida jurídico (não-team, esporádico: mar 92,45) | Base_Resultado tem linha por advogado "Seguro de Vida" (hoje `manual`); baixa prioridade |
...
Mar/Abr are one-offs. Full per-line attribution, now down to the account:
`docs/DIFERENCAS_ACUMULADO_2026.md`, regenerated by `scripts/build_diferencas_doc.py`.
...
Um diff anterior (`DIFF_JAN_ABR_2026.md`, removido 2026-08-05 — superseded por
`DIFERENCAS_ACUMULADO_2026.md`) listava r34/r35/r43/r47/r51/r54 como *"sem lançamento
