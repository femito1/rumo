...
def test_monthly_budget_splits_annual_evenly():
    entries = [BudgetEntry("mbc", 2026, "institucional", "faturamento", 1200.0)]
    monthly = monthly_budget(entries)
    assert monthly["institucional"]["faturamento"] == 100.0
...
def test_budget_source_emits_orcamento_tab():
    ...
    fat = next(r for r in tab["rows"] if r["Linha"] == "Faturamento")
