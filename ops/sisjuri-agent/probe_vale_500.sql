...
-- NOTE: the single concatenated SELECT expression is position 1; ORDER BY must use
-- the underlying columns, NOT positions 1,2 (ORA-01785 on 11g).
SET DEFINE OFF
SET PAGESIZE 0
SET LINESIZE 32767
SET TRIMOUT ON
...
SELECT 'C|'||TO_CHAR(cp.CPGDVECTO,'YYYY-MM')
 ...
 ORDER BY TO_CHAR(cp.CPGDVECTO,'YYYY-MM');
